...
-- Autores: Grupo FacGESC — UNICID ADS 2026
-- ================================================================

-- ================================================================
-- FASE 0 — RESET / CRIAÇÃO DO BANCO
-- Executar reset.sql separadamente para zerar.
-- ================================================================
...
-- ================================================================
-- FASE 1 — DDL: ESTRUTURA DO BANCO
-- Organização: Base Compartilhada → RH → Acadêmico → Financeiro
-- ================================================================
...
CREATE TABLE IF NOT EXISTS tb_cargo_funcional (
...
CREATE TABLE IF NOT EXISTS tb_setor_institucional (
...
CREATE TABLE IF NOT EXISTS tb_alocacao_setor (
  fk_rf              INT     NOT NULL,
...
-- Surrogate mantido: colaborador pode tirar férias fracionadas
-- ------------------------------------------------------------

CREATE TABLE IF NOT EXISTS tb_periodo_ferias (
...
CREATE TABLE IF NOT EXISTS tb_curso_graduacao (
...
CREATE TABLE IF NOT EXISTS tb_disciplina_catalogo (
...
CREATE TABLE IF NOT EXISTS tb_historico_situacao_estudante (
...
CREATE TABLE IF NOT EXISTS tb_responsavel_financeiro (
...
CREATE TABLE IF NOT EXISTS tb_estudante_responsavel (
  fk_ra                 INT      NOT NULL,
...
CREATE TABLE IF NOT EXISTS tb_oferta_disciplina (
...
CREATE TABLE IF NOT EXISTS tb_matricula_estudante (
  fk_ra                INT           NOT NULL,
...
CREATE TABLE IF NOT EXISTS tb_avaliacao_programada (
...
CREATE TABLE IF NOT EXISTS tb_nota_avaliacao (
  fk_ra              INT          NOT NULL,
...
-- UNIQUE (fk_oferta, data_aula): impede aula duplicada no mesmo dia/turma
-- ------------------------------------------------------------

CREATE TABLE IF NOT EXISTS tb_aula_registrada (
...
CREATE TABLE IF NOT EXISTS tb_frequencia_aula (
  fk_ra               INT          NOT NULL,
...
CREATE TABLE IF NOT EXISTS tb_ocorrencia_disciplinar (
...
CREATE TABLE IF NOT EXISTS tb_tabela_mensalidade (
  fk_curso             INT           NOT NULL,
  fk_ano_letivo        INT           NOT NULL,
  fk_semestre          INT           NOT NULL,
...
CREATE TABLE IF NOT EXISTS tb_bolsa_desconto (
...
CREATE TABLE IF NOT EXISTS tb_contrato_academico (
...
-- Correção 3FN: valor_total movido para view vw_parcela_com_total
-- ------------------------------------------------------------

CREATE TABLE IF NOT EXISTS tb_parcela_mensalidade (
...
-- Uma parcela pode ter múltiplos recebimentos (pagamento parcelado)
-- ------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS tb_controle_inadimplencia (
...
CREATE TABLE IF NOT EXISTS tb_fornecedor_servico (
...
CREATE TABLE IF NOT EXISTS tb_despesa_operacional (
...
CREATE TABLE IF NOT EXISTS tb_pagamento_despesa (
...
-- Campos calculados ficam aqui, não nas tabelas
-- ================================================================

-- Valor total de cada parcela (nominal + multa + juros)
...
-- Salário líquido de cada colaborador por competência
...
-- Frequência por matrícula (total aulas, faltas, percentual presença)
...
-- Vagas disponíveis por oferta de disciplina
...
-- Coeficiente de Rendimento por estudante
-- CR = média ponderada da nota final por disciplina × créditos
-- Considera apenas disciplinas concluídas (aprovado ou reprovado)
CREATE OR REPLACE VIEW vw_cr_por_estudante AS
SELECT
  m.fk_ra,
  ROUND(
    SUM(m.nota_final * d.num_creditos)
...
-- Resumo de inadimplência ativa por aluno
...
-- ================================================================
-- ÍNDICES — Fase 5: Performance
-- Criados em colunas usadas em JOIN, filtro e ordenação
-- ================================================================

-- Módulo RH
CREATE INDEX IF NOT EXISTS idx_colaborador_cargo   ON tb_colaborador(fk_cargo);

-- Módulo Acadêmico
CREATE INDEX IF NOT EXISTS idx_estudante_curso      ON tb_estudante(fk_curso);
CREATE INDEX IF NOT EXISTS idx_estudante_situacao   ON tb_estudante(situacao);
CREATE INDEX IF NOT EXISTS idx_estudante_risco      ON tb_estudante(flag_risco_evasao);
CREATE INDEX IF NOT EXISTS idx_oferta_disciplina    ON tb_oferta_disciplina(fk_disciplina);
CREATE INDEX IF NOT EXISTS idx_oferta_docente       ON tb_oferta_disciplina(fk_rf_docente);
CREATE INDEX IF NOT EXISTS idx_matricula_oferta     ON tb_matricula_estudante(fk_oferta);
CREATE INDEX IF NOT EXISTS idx_nota_avaliacao       ON tb_nota_avaliacao(fk_avaliacao);
CREATE INDEX IF NOT EXISTS idx_frequencia_aula      ON tb_frequencia_aula(fk_aula);

-- Módulo Financeiro
CREATE INDEX IF NOT EXISTS idx_parcela_contrato     ON tb_parcela_mensalidade(fk_contrato);
CREATE INDEX IF NOT EXISTS idx_parcela_situacao     ON tb_parcela_mensalidade(situacao_parcela);
CREATE INDEX IF NOT EXISTS idx_parcela_vencimento   ON tb_parcela_mensalidade(data_vencimento);
CREATE INDEX IF NOT EXISTS idx_recebimento_parcela  ON tb_recebimento(fk_contrato, fk_numero_parcela);
CREATE INDEX IF NOT EXISTS idx_bolsa_ra             ON tb_bolsa_desconto(fk_ra);
CREATE INDEX IF NOT EXISTS idx_despesa_setor        ON tb_despesa_operacional(fk_setor);

-- ================================================================
-- FASE 2 — DML: CARGA DE DADOS (Idempotente via INSERT IGNORE)
-- ================================================================

-- Contagem ANTES da carga — deve ser igual após reexecução
...
-- RH: Cargos, Setores, Colaboradores, Docentes
...
-- Acadêmico: Cursos, Disciplinas, Períodos, Estudantes
...
-- Financeiro: Tabelas, Contratos, Parcelas, Recebimentos
...
-- Contagem DEPOIS — deve ser IDÊNTICA à contagem ANTES se executado pela 2ª vez
...
-- ================================================================
-- FASE 3 — OLTP: Consultas simples
-- ================================================================

-- Listagem completa de estudantes com curso e situação
SELECT
  e.pk_ra,
...
-- Parcelas com valor total calculado inline (demonstra 3FN)
SELECT
  p.fk_contrato,
  p.numero_parcela,
  p.valor_nominal,
...
-- Folha de pagamento com salário líquido calculado inline (demonstra 3FN)
SELECT
  CONCAT(p.primeiro_nome, ' ', p.sobrenome) AS nome,
  ca.nome_cargo,
  f.salario_bruto,
...
-- ================================================================
-- FASE 3 — OLTP: Subselects e Subqueries correlacionadas
-- ================================================================
...
-- Demonstra subquery correlacionada com agregação interna
SELECT
  CONCAT(p.primeiro_nome, ' ', p.sobrenome) AS nome,
  e.pk_ra
FROM tb_estudante e
...
-- Total pago por aluno (subquery de agregação)
SELECT
  CONCAT(p.primeiro_nome, ' ', p.sobrenome) AS nome,
...
-- Disciplinas com alunos matriculados (GROUP BY + HAVING)
SELECT
  d.nome_disciplina,
...
-- Alunos com parcelas vencidas (subquery correlacionada no SELECT + GROUP BY correto)
SELECT
  CONCAT(p.primeiro_nome, ' ', p.sobrenome) AS nome,
  e.pk_ra,
  (SELECT COUNT(*)
...
-- Demonstração das propriedades: Atomicidade, Consistência, Durabilidade
-- ================================================================

-- CENÁRIO 1: ROLLBACK — a transação é revertida por completo
START TRANSACTION;
...
-- Validação: parcela deve continuar 'em_aberto' após ROLLBACK
SELECT fk_contrato, numero_parcela, situacao_parcela, data_pagamento
FROM tb_parcela_mensalidade
WHERE fk_contrato = 1 AND numero_parcela = 4;

-- CENÁRIO 2: COMMIT — operação persiste normalmente
START TRANSACTION;
INSERT INTO tb_recebimento
  (fk_contrato, fk_numero_parcela, valor_recebido, data_recebimento, modalidade_pagamento, data_cadastro)
VALUES
...
-- Validação: parcela deve estar 'paga' após COMMIT
SELECT fk_contrato, numero_parcela, situacao_parcela, data_pagamento
FROM tb_parcela_mensalidade
WHERE fk_contrato = 1 AND numero_parcela = 4;

-- CENÁRIO 3: Transação multi-tabela com ROLLBACK (atomicidade avançada)
-- Simula pagamento + atualização de inadimplência + falha → tudo reverte
START TRANSACTION;
INSERT INTO tb_recebimento
  (fk_contrato, fk_numero_parcela, valor_recebido, data_recebimento, modalidade_pagamento, data_cadastro)
VALUES
...
-- Validação: parcela do contrato 2 deve continuar 'vencida' — o ROLLBACK reverteu tudo
SELECT fk_contrato, numero_parcela, situacao_parcela
FROM tb_parcela_mensalidade
WHERE fk_contrato = 2 AND numero_parcela = 3;

-- ================================================================
-- FASE 5 — PERFORMANCE: EXPLAIN (antes e depois dos índices)
-- Os índices foram criados na seção de índices acima
-- ================================================================

-- EXPLAIN 1: Query financeira com filtro por situacao_parcela
-- O índice idx_parcela_situacao evita full scan em tb_parcela_mensalidade
EXPLAIN SELECT
  CONCAT(p.primeiro_nome, ' ', p.sobrenome) AS nome,
  c.nome_curso,
...
-- EXPLAIN 2: Query acadêmica com JOIN entre oferta, disciplina e matrícula
-- O índice idx_matricula_oferta e idx_oferta_disciplina são usados aqui
EXPLAIN SELECT
  d.nome_disciplina,
...
-- ================================================================
-- FASE 4 — OLAP: MODELO ESTRELA (Star Schema)
-- Surrogate keys em todas as dimensões (padrão Kimball)
-- Chave natural mantida como UNIQUE para idempotência do ETL
-- ================================================================
...
-- Fato Pagamento: granularidade = 1 linha por parcela por aluno por data de vencimento
...
-- Fato Desempenho: granularidade = 1 linha por avaliação por aluno
...
-- Fato Frequência: granularidade = 1 linha por aula por aluno
...
-- Índices nas FKs das tabelas fato (melhora performance das queries analíticas)
CREATE INDEX IF NOT EXISTS idx_fato_pg_sk_tempo   ON fato_pagamento(sk_tempo);
CREATE INDEX IF NOT EXISTS idx_fato_pg_sk_aluno   ON fato_pagamento(sk_aluno);
CREATE INDEX IF NOT EXISTS idx_fato_pg_sk_curso   ON fato_pagamento(sk_curso);
CREATE INDEX IF NOT EXISTS idx_fato_des_sk_aluno  ON fato_desempenho(sk_aluno);
CREATE INDEX IF NOT EXISTS idx_fato_des_sk_disc   ON fato_desempenho(sk_disciplina);
CREATE INDEX IF NOT EXISTS idx_fato_freq_sk_aluno ON fato_frequencia(sk_aluno);

-- ================================================================
-- ETL — Extração, Transformação e Carga
-- Ordem obrigatória: dim_tempo → dimensões → fatos
-- ================================================================

-- ETL 1: dim_tempo a partir das datas de vencimento das parcelas
...
-- ETL 2: dim_tempo a partir das datas de aula (fato_frequencia)
INSERT IGNORE INTO dim_tempo (data_completa, dia, mes, nome_mes, trimestre, semestre, ano, dia_semana)
...
-- ETL 3: dim_tempo a partir das datas de aplicação de avaliações (fato_desempenho)
INSERT IGNORE INTO dim_tempo (data_completa, dia, mes, nome_mes, trimestre, semestre, ano, dia_semana)
SELECT DISTINCT data_aplicacao,
  DAY(data_aplicacao), MONTH(data_aplicacao),
  CASE MONTH(data_aplicacao)
    WHEN 1  THEN 'Janeiro'   WHEN 2  THEN 'Fevereiro' WHEN 3  THEN 'Marco'
    WHEN 4  THEN 'Abril'     WHEN 5  THEN 'Maio'      WHEN 6  THEN 'Junho'
    WHEN 7  THEN 'Julho'     WHEN 8  THEN 'Agosto'    WHEN 9  THEN 'Setembro'
    WHEN 10 THEN 'Outubro'   WHEN 11 THEN 'Novembro'  WHEN 12 THEN 'Dezembro'
  END,
  QUARTER(data_aplicacao),
  CASE WHEN MONTH(data_aplicacao) <= 6 THEN 1 ELSE 2 END,
  YEAR(data_aplicacao),
  DAYNAME(data_aplicacao)
FROM tb_avaliacao_programada;

-- ETL 4: dim_aluno (GROUP BY completo para evitar non-determinism no MySQL 5.7+)
...
-- ETL 8: fato_pagamento (1 linha por parcela — LEFT JOIN em recebimento traz valor pago)
...
-- ================================================================
-- VALIDAÇÃO ETL — SUM(OLTP) deve ser exatamente igual a SUM(OLAP)
-- etl_*_ok = 1 significa que o ETL está correto
-- ================================================================

-- Validação 1: soma dos valores nominais das parcelas
SELECT
...
-- Validação 2: contagem de registros de notas
SELECT
...
-- Validação 3: contagem de registros de frequência
SELECT
...
-- Validação 4: integridade do Star Schema — nenhuma SK deve ser NULL
SELECT
  (SELECT COUNT(*) FROM fato_pagamento  WHERE sk_tempo IS NULL) AS pag_sk_tempo_nulo,
  (SELECT COUNT(*) FROM fato_pagamento  WHERE sk_aluno IS NULL) AS pag_sk_aluno_nulo,
  (SELECT COUNT(*) FROM fato_desempenho WHERE sk_tempo IS NULL) AS des_sk_tempo_nulo,
  (SELECT COUNT(*) FROM fato_frequencia WHERE sk_tempo IS NULL) AS freq_sk_tempo_nulo;
-- Todos os valores acima devem ser 0

-- ================================================================
-- CONSULTAS ANALÍTICAS OLAP (Star Schema)
-- ================================================================
...
SELECT 'CONTAGEM EXATA POR TABELA' AS info;
...
SELECT 'FacGESC — Script executado com sucesso!' AS status;

-- ================================================================
-- FIM DO SCRIPT
-- FacGESC — UNICID ADS 2026
-- ================================================================
